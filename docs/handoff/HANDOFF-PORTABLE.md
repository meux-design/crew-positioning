...
This brief is written for any AI coding assistant. Read it in full before writing code.

**Working agreement — treat these as hard constraints, not suggestions:**

1. Do not write implementation code until the specification for the current work unit exists and has been reviewed.
2. When a requirement here is ambiguous or contradicted by what you find in the API docs, stop and ask. Do not guess and proceed.
...
5. When you finish a work unit, state what you built, what you skipped, and what you were unsure about.
...
Build in this order. Each unit is specified, reviewed, then implemented.

1. **Project scaffold** — Next.js, Tailwind, Prisma, environment variable validation, CI
2. **External API client** — typed client, Zod schemas, cache layer, error taxonomy
3. **Search flow** — form, validation, route handler, results list
4. **Ranking engine** — fitScore computation, explainability payload
5. **Saved searches** — persistence, list view, re-run
6. **Result detail** — slide-over, fare breakdown
7. **Accessibility pass** — audit against §10, remediate

### Specification format

Before implementing each unit, produce a specification document containing:

- **Intent** — one paragraph on what changes and why
- **Requirements** — numbered, testable statements. Each uses "the system shall…" phrasing and describes observable behaviour, not implementation.
- **Scenarios** — for each requirement, at least one concrete case in given/when/then form
- **Task list** — ordered, checkable implementation steps
- **Out of scope** — what this unit deliberately does not do

Keep specifications and implementation in separate files. The specification is a reviewable artefact in its own right; it is part of the portfolio deliverable, not scaffolding to be discarded.

## 13. Definition of done

- [ ] All seven work units specified, reviewed, and implemented
- [ ] Lighthouse accessibility score at least 95 on search and results
- [ ] Zero TypeScript errors under strict mode
- [ ] One Playwright test covering search → results → detail
- [ ] Deployed to a public URL
- [ ] README explains the problem, the approach, and the tradeoffs
- [ ] Works on a 375px viewport — this is a mobile tool first

## 14. Open questions

Raise these rather than guessing:

- Does seats.aero return arrival times directly, or must they be derived from flight schedules? This determines whether the core "arrive before" constraint is implementable as specified. Resolve this first — it is load-bearing for the entire concept.
- Cache TTL, which depends on the live and cached endpoint behaviour and on rate limit headroom
- Whether to seed a demo user or add lightweight authentication
